...
import { ArrowLeft, Landmark, Wallet, PlusCircle, Calendar as CalendarIcon, ChevronDown, ChevronUp, MoreHorizontal, Pencil, Trash2, Briefcase, Combine, ArrowUpCircle, ArrowDownCircle, Scale, FileText, Banknote } from "lucide-react"
...
import { format, isWithinInterval, startOfMonth, endOfMonth, getYear, setMonth, getMonth, startOfDay } from "date-fns"
...
export default function TourAccountancyPage() {
    ...
    const handleUpdateTransaction = async () => {
        ...
        try {
            const { id, createdAt, ...dataToUpdate } = editingTransaction;
            await updateTourTransaction(id, {
                ...dataToUpdate,
                                    ...
                                    <AccordionTrigger>
                                        <div className="flex justify-between w-full pr-4">
                                            ...
                                            <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs">
                                                <span className="text-green-600">ຮັບ: {currencies.map(c => summary.income[c] > 0 ? `${formatCurrency(summary.income[c])} ${c.toUpperCase()}`: '').filter(Boolean).join(' / ')}</span>
                                                <span className="text-red-600">ຈ່າຍ: {currencies.map(c => summary.expense[c] > 0 ? `${formatCurrency(summary.expense[c])} ${c.toUpperCase()}`: '').filter(Boolean).join(' / ')}</span>
                                            </div>
                                        </div>
                                    </AccordionTrigger>
                                    <AccordionContent>
