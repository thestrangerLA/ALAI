// src/app/tour/reports/program-summary/page.tsx
"use client"
...
export default function ProgramSummaryPage() {
    ...
    const reportsData: YearlyReport[] = useMemo(() => {
        ...
        const filteredPrograms = programs.filter(p => {
            ...
            return isWithinInterval(new Date(p.date), { start: startDate, end: endDate });
        });

        const programReports: ProgramReport[] = filteredPrograms.map(p => {
            ...
            const totalCost = programCosts.reduce((acc, item) => {
                currencies.forEach(c => acc[c] += (item as any)[c.toLowerCase()] as number || 0);
                return acc;
            ...
            return { ...(p as any), totalCost, totalIncome, profit };
        });

        const groupedByYear = programReports.reduce((acc, report) => {
            const year = getYear(new Date(report.date));
            if (!acc[year]) {
                                            ...
                                            <AccordionTrigger className="hover:bg-muted/50 px-4 rounded-md">
                                                <div className="flex justify-between w-full pr-4 items-center">
                                                    <div className="text-left">
                                                        ...
                                                        <p className="text-sm text-muted-foreground">{program.tourDates} | {program.date ? format(new Date(program.date), "dd MMM yyyy") : ''}</p>
                                                    </div>
